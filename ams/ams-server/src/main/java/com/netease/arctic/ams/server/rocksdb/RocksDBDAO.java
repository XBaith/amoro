...
public class RocksDBDAO {
  ...
  public RocksDBDAO(String rocksDBBasePath) {
  ...
  public <T extends Serializable> Collection<T> keys(String cf) {
    Collection<T> keys = new ArrayList<T>();
    RocksIterator itr = rocksDB.newIterator(managedHandlesMap.get(cf));
    itr.seekToFirst();
    while (itr.isValid()) {
      keys.add((T) SerializationUtils.deserialize(itr.key()));
      itr.next();
    }
    return keys;
  }
  ...
  public <T extends Serializable> void put(String columnFamilyName, String key, T value) {
    try {
      LOG.info(String.format("Put %s.%s to %s", columnFamilyName, key, rocksDBBasePath));
      byte[] payload = serializePayload(value);
  ...
  public <K extends Serializable, T extends Serializable> void put(String columnFamilyName, K key, T value) {
    try {
      LOG.info(String.format("Put %s.%s to %s", columnFamilyName, key, rocksDBBasePath));
      byte[] payload = serializePayload(value);
      getRocksDB().put(managedHandlesMap.get(columnFamilyName), new WriteOptions(),
  ...
  public <K extends Serializable> void delete(String columnFamilyName, K key) {
    try {
      getRocksDB().delete(managedHandlesMap.get(columnFamilyName),
          new WriteOptions(), SerializationUtils.serialize(key));
    } catch (Exception e) {
      throw new RuntimeException(e);
    }
  }
  ...
  public <T extends Serializable> T get(String columnFamilyName, String key) {
    Preconditions.checkArgument(!closed);
    try {
      LOG.info(String.format("Get %s.%s from %s", columnFamilyName, key, rocksDBBasePath));
